...
export interface ChartSeries {
	...
	classAvgValues: number[] | null;
}

/**
 * Builds cumulative weighted-% series for the progress chart: at each
 * component (in entry order), how much weighted % has been banked so
 * far, for the student vs the class average. Invalid components add 0.
 */
export function buildChartSeries(course: Course): ChartSeries {
	const labels = course.components.map((c) => c.type);

	let runningStudent = 0;
	const studentValues = course.components.map((c) => {
		const score = computeComponentScore(c, 'student');
		if (score.valid) {
			runningStudent += score.pctValue * ((Number(c.weightage) || 0) / 100);
		}
		return runningStudent;
	});

	if (course.mode !== 'relative') {
		return { labels, studentValues, classAvgValues: null };
	}

	let runningClassAvg = 0;
	const classAvgValues = course.components.map((c) => {
		const score = computeComponentScore(c, 'classAvg');
		if (score.valid) {
			runningClassAvg += score.pctValue * ((Number(c.weightage) || 0) / 100);
		}
		return runningClassAvg;
	});

	return { labels, studentValues, classAvgValues };
}